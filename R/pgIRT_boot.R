...
pgIRT_boot <- function(fit, boot = 100, verbose = NULL) {
  if (class(fit) != "pgIRT") stop("Please supply the fit from `pgIRT`.")
  ...
  if (fit$model == "bin") {
    ...
    for (b in 1:boot) {
      ...
      if (!is.null(names(fit$parameter$theta))) {
        names(theta_boot[[b]]) <- names(fit$parameter$theta)
      }
      alpha_boot[[b]] <- fit_boot$parameter$alpha
      beta_boot[[b]] <- fit_boot$parameter$beta
      
      if (b %% verbose == 0) {
        cat("Boostrap", b, "DONE :", crayon::yellow(round(proc.time()[3] - stime, 3), "sec\n"))
      }
    }
  ...
  if (fit$model == "bin_dyn") {
    ...
    for (b in 1:boot) {
      ...
      theta_boot[[b]] <- fit_boot$parameter$theta
      if (!is.null(rownames(fit$parameter$theta))) {
        rownames(theta_boot[[b]]) <- rownames(fit$parameter$theta)
      }
      alpha_boot[[b]] <- fit_boot$parameter$alpha
      beta_boot[[b]] <- fit_boot$parameter$beta
